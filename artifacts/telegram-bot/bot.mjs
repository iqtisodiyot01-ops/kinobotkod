 ...
 * 1. npm install
 * 2. .env faylini sozlang (pastda ko'rsatilgan)
 * 3. node bot.mjs
 * 
...
bot.onText(/\/start(.*)/, async (msg, match) => {
  ...
  await bot.sendMessage(chatId, `👋 ${welcomeMsg}\n\nSalom, ${msg.from.first_name}!`, buildMainKeyboard());
});
...
bot.onText(/\/admin/, async (msg) => {
  ...
  await bot.sendMessage(msg.chat.id, 
    ...
    "🎬 Admin panel orqali kinolar boshqarish mumkin",
...
bot.on("message", async (msg) => {
  ...
  // Numeric search — look up by code (if admin sets movie code)
  if (/^\d+$/.test(text)) {
...
bot.on("callback_query", async (query) => {
  ...
  if (data === "check_subscription") {
    if (await checkChannelMembership(telegramId)) {
      ...
      await bot.answerCallbackQuery(query.id, { text: "Hali a'zo bo'lmagansiz!", show_alert: true });
    }
    return;
  }
  
  if (data.startsWith("movie_")) {
    const movieId = parseInt(data.replace("movie_", ""));
    const movie = await query_db("SELECT * FROM movies WHERE id = $1 AND is_active = true", [movieId]);
...
// Fix: need to rename local query function for callback handler
